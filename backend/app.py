...
def generate_keys():
    ...
    return jsonify({
        ...
        "optimizations": TIERS[tier]["optimizations"]
    })
...
def validate_license():
    ...
    if row["expires_at"]:
        try:
            ...
            return jsonify({
                "valid": False,
                "reason": "Data de expirare invalida"
            }), 500
    ...
    if row["hwid"] is None:
        ...
        return jsonify({
            "valid": False,
            "reason": "Cheia este deja activa pe alt dispozitiv"
        }), 403
    ...
    if tier_info is None:
        return jsonify({
            "valid": False,
            "reason": "Nivel de licenta invalid"
        }), 500
...
@app.route("/api/license/tiers", methods=["GET"])
def license_tiers():
    result = {}
    ...
    for tier_name, tier in TIERS.items():
        result[tier_name] = {
            ...
            "optimizations": [
                {
                    "id": optimization,
                    "name": OPTIMIZATIONS_CATALOG.get(
                        optimization,
                        optimization
                    )
                }
                for optimization in tier["optimizations"]
            ]
        }
